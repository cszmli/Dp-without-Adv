...
class MultiWozStateEncoder(object):
    ...
    def encode(self, state):
        ...
        # print(info_vector.shape)
        
        request_vector = self.get_request_state(state['request_state'])
        # print(request_vector.shape)
        
        user_act_vector = self.get_user_act_state(state['user_action'])
        # print(user_act_vector.shape)
        
        history_vector = self.get_history_state(state['history'])
        # print(history_vector.shape)
        
        encoder_out = np.concatenate((db_vector, book_vector, info_vector, request_vector, user_act_vector, history_vector))
    ...
    def get_history_state(self, history):
        # count how many time the last act has been repeated.
        history_vector = []
        # print("s==================================")
        # print(history)
        # print("s==================================")
        ...
        # print("history state")
        # print(len(history_vector))
        return history_vector

    def get_user_act_state(self, user_act):
        user_act_vector = []
        # print("user act state")
        count = 0
        for domain in REF_SYS_DA:
            count += len(REF_SYS_DA[domain])
            # print(count)
    ...
    def get_info_state(self, belief_state):
        ...
        for domain in domains:
            domain_active = False
            # print('book', domain, len(belief_state[domain]['book'].keys()))
            booking = []
            ...
            # print('semi', len(belief_state[domain]['semi']), ' * [0,0,0]')
