...
import math
import laed.normalizer.delexicalize as delex
from laed.utils import get_tokenize
from collections import Counter
from nltk.util import ngrams
from laed.dataset.corpora import SYS, USR, BOS, EOS
import json
from laed.normalizer.delexicalize import normalize
import sqlite3
import os
import random
import logging
...
class MultiWozDB(object):
    ...
    dbs = {}
    CUR_DIR = os.path.dirname(__file__).replace('laed', '')
    print(CUR_DIR)

    for domain in domains:

        # db = os.path.join(CUR_DIR, 'data/norm-multi-woz/db/{}-dbase.db'.format(domain))
        db ="/home/t-zimli/irl/NeuralDialog-LAED/data/norm-multi-woz/db/{}-dbase.db".format(domain)
...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def add_example(self, ref, hyp, domian='default'):
        self.labels.append(ref)
        self.hyps.append(hyp)
