...
def generate(model, data_feed, config, evaluator, num_batch=1, print_flag=True, dest_f=None):
    model.eval()
    ...
    while True:
        ...
        for b_id in range(pred_labels.shape[0]):
            ...
            for i in range(ctx_size):
                temp, _ = engine.get_sent(model, de_tknize, ctx[:, i, 1:], b_id, ct=True)
                if temp:
                    ctx_str.append(temp)
...
def gen_with_cond(model, data_feed, config, num_batch=1, dest_f=None):
    ...
    while True:
        ...
        for b_id in range(true_labels.shape[0]):
            ctx_str = []
            for i in range(ctx_size):
                temp, _ = engine.get_sent(model, de_tknize, ctx[:, i, :], b_id, stop_eos=False)
                if temp:
                    ctx_str.append(temp)
            ctx_str = '<t>'.join(ctx_str)
