...
import torch
from laed.dataset.corpora import PAD, EOS, EOT, EOTSELF
...
def get_sent(model, de_tknize, data, b_id, attn=None, attn_ctx=None, stop_eos=True, stop_pad=True, ct=False):
    ws = []
    attn_ws = []
    ...
    for t_id in range(data.shape[1]):
        ...
        if (stop_eos and w in [EOT, EOS]) or (stop_pad and w == PAD):
            if ct==False:
                if w == EOT:
                    ws.append(w)
                break
                ...
                if valid_loss < best_valid_loss:
                    ...
                    if config.save_model:
                        ...
                        torch.save({
                                    ...
                                    "utt_encoder": model.utt_encoder.state_dict(),
                                    "ctx_encoder": model.ctx_encoder.state_dict(),
                                    "p_y":model.p_y.state_dict(),
                                    "p_fc1":model.p_fc1.state_dict(),
                                    "x_encoder": model.x_encoder.state_dict(),
                                    "x_embedding": model.x_embedding.state_dict(),
                                    "q_y": model.q_y.state_dict()
...
def generate(model, data_feed, config, evaluator, num_batch, dest_f=None):
    model.eval()
